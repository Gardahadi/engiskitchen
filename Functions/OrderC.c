...
void AddOrder(int j)
{
    if (IsEmptyOrder(R.TabOfOrder)){
        ...
        OrderResto[NeffOrd(R.TabOfOrder)].NomorMeja = (RN*3-3)+j+1;
    }
    else{
        int i = 1;
        boolean SudahAda = false;
        while ((i <= NeffOrd(R.TabOfOrder) && (!SudahAda))){
            if ((TableArray(RN,j).C.Makanan) == OrderResto[i].CustomerOrder){
                SudahAda = true;
            }
        }
        if (!SudahAda){
            NeffOrd(R.TabOfOrder) += 1;
            OrderResto[NeffOrd(R.TabOfOrder)].CustomerOrder = TableArray(RN,j).C.Makanan;
            OrderResto[NeffOrd(R.TabOfOrder)].NomorMeja = (RN*3-3)+j;
        }
    }
}
