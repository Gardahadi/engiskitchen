...
typedef struct {
	CUSTOMER CustomerOrder; /* Menyimpan  */
	int NomorMeja; /*Menyimpan Nomor meja yang memesan */
} Orderan;

typedef struct {
	Orderan TOrd[IdxMaxOrder+1]; /* memori tempat penyimpan elemen (container) */
...
IdxTypeOrder GetLastIdxOrder (TabInt T);
/* Prekondisi : Tabel T tidak kosong */
...
void AddAsLastElOrder (TabOrder * T, ElTypeOrder X);
